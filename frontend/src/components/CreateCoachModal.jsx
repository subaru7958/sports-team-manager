...
const CreateCoachModal = ({ isOpen, onClose, onCoachCreated, defaultCategory }) => {
    const { team } = useAuth();
    ...
    const [category, setCategory] = useState(defaultCategory || "senior");
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.post("/coaches/new", payload);
            if (response.data.success) {
                onCoachCreated(response.data.coach);
                onClose();
                // Reset form
                setFullName("");
                setPhone("");
            ...
            setError(err.response?.data?.message || "Failed to register coach");
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col w-full max-w-[500px] animate-in zoom-in-95 duration-300">
                ...
                <div className="px-8 pt-8 pb-4 flex justify-between items-start">
                    <div className="flex flex-col gap-1">
                        <h2 className="text-[#111817] text-2xl font-black">Register Coach</h2>
                        <p className="text-slate-500 text-sm font-medium">Add a new member to your technical staff.</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="px-8 py-4 space-y-5 overflow-y-auto max-h-[75vh]">
                    ...
                    <div className="flex flex-col items-center gap-4 pb-2">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.1em]">Staff Portrait (Optional)</p>
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-6 border-t border-slate-50 mt-4">
                        ...
                        <Button loading={loading} type="submit" className="!min-w-[140px] shadow-lg" style={{ backgroundColor: primaryColor }}>Register Coach</Button>
                    </div>
                </form>
            </div>
        </div>
